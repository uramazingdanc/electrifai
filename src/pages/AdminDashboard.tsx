...
import { AlertTriangle, Users, Zap, DollarSign, MapPin, TrendingUp, TrendingDown } from 'lucide-react';
const AdminDashboard = () => {
  ...
  const theftAlerts = [{
    id: 1,
    region: 'Calabarzon',
    substation: 'Laguna Substation 3',
    anomalyLevel: 85,
    timestamp: '2 hours ago'
  }, {
    id: 2,
    region: 'Western Visayas',
    substation: 'Iloilo Feeder 7',
    anomalyLevel: 76,
    timestamp: '4 hours ago'
  }, {
    id: 3,
    region: 'Davao Region',
    substation: 'Davao City Grid 2',
    anomalyLevel: 92,
    timestamp: '1 day ago'
  }, {
    id: 4,
    region: 'Central Luzon',
    substation: 'Pampanga Line 5',
    anomalyLevel: 68,
    timestamp: '1 day ago'
  }];
  const pieData = [{
  ...
  return <MainLayout userRole="admin">
      <div className="container mx-auto">
        <h1 className="text-2xl font-bold mb-6">Administrator Dashboard</h1>
        
        {/* System Statistics */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
          {systemStats.map((stat, index) => <Card key={index} className="shadow-card">
              <CardContent className="pt-6 px-[12px]">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="text-sm font-medium text-gray-500">{stat.title}</p>
                    <p className="text-2xl font-bold mt-1">{stat.value}</p>
                    <div className="flex items-center mt-1">
                      {stat.isIncrease ? <TrendingUp className="h-4 w-4 mr-1 text-electricred" /> : <TrendingDown className="h-4 w-4 mr-1 text-electricgreen" />}
                ...
                <div className="h-[400px] flex flex-col justify-center">
                  ...
                  <div className="flex justify-center space-x-4 mt-4">
                    {pieData.map((entry, index) => <div key={index} className="flex items-center">
                        <div className="w-3 h-3 mr-1 rounded-sm" style={{
          ...
          <Card className="shadow-card">
            ...
            <CardContent>
              <div className="space-y-4">
                {theftAlerts.map(alert => <div key={alert.id} className="border rounded-lg p-4">
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="font-medium">{alert.substation}</h3>
                        <p className="text-sm text-gray-500">{alert.region}</p>
                      </div>
                      <div className={`py-1 px-2 rounded text-white text-xs ${alert.anomalyLevel > 80 ? 'bg-electricred' : alert.anomalyLevel > 60 ? 'bg-amber-500' : 'bg-electricgreen'}`}>
              ...
              <div className="h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={regionData}>
                    ...
                    <Tooltip />
                    <Legend />
                    <Line type="monotone" dataKey="theft" name="Theft Indicators (%)" stroke="#CC3300" activeDot={{
                    r: 8
                  }} />
                  </LineChart>
                </ResponsiveContainer>
