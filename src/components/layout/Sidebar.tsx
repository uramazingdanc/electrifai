...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside className={cn("bg-[#f2f6fa] fixed inset-y-0 left-0 z-10 flex flex-col transition-all duration-300 border-r", open ? "w-64" : "w-20")}>
        ...
        <ul className="space-y-1 px-2">
          {filteredItems.map(item => (
            <li key={item.path}>
              <Link 
                to={item.path} 
                className="flex items-center p-2 rounded-md hover:bg-white group"
              >
                <item.icon className="h-5 w-5 text-electricblue" />
