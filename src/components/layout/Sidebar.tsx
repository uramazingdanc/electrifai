...
export const Sidebar: React.FC<SidebarProps> = ({
            ...
            <nav className="flex-1 overflow-y-auto py-4">
              <ul className="space-y-1 px-2">
                {filteredItems.map(item => <li key={item.path}>
                    <Link to={item.path} className="flex items-center p-2 rounded-md hover:bg-electricgray group" onClick={onClose}>
  ...
  return <aside className={cn("bg-white fixed inset-y-0 left-0 z-10 flex flex-col transition-all duration-300 border-r", open ? "w-64" : "w-20")}>
      ...
      <nav className="flex-1 overflow-y-auto py-4">
        <ul className="space-y-1 px-2">
          {filteredItems.map(item => <li key={item.path}>
              <Link to={item.path} className="flex items-center p-2 rounded-md hover:bg-electricgray group">
                <item.icon className="h-5 w-5 text-electricblue" />
      ...
      <div className="p-4 border-t">
        <Link to="/profile" className={cn("flex items-center p-2 rounded-md hover:bg-electricgray", !open && "justify-center")}>
          <User className="h-5 w-5 text-gray-600" />
          {open && <span className="ml-3">Profile</span>}
        </Link>
        <button className={cn("flex items-center p-2 rounded-md hover:bg-electricgray w-full mt-2", !open && "justify-center")}>
          <LogOut className="h-5 w-5 text-gray-600" />
